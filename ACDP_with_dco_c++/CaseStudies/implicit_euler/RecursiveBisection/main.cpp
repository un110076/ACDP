...
struct StaticEvolutionRecursiveBisection_Target : ACDP_EvolutionStep {
  ...
  StaticEvolutionRecursiveBisection_Target(
      DCO_TT *context_tape, const double& dt, const double &eps, const int& n
...
void implicit_euler(DCO_A& x, std::vector<DCO_A>& p, 
                    const double& T, const double &eps, const int& ncp) {
  int n=p.size(); double dt=T/n;
  DCO_TT* context_tape=dco::tape(x);
  ACDP_StaticEvolutionRecursiveBisection *e= new ACDP_StaticEvolutionRecursiveBisection(context_tape,new StaticEvolutionRecursiveBisection_Target(context_tape,dt,eps,n),n+1,1,n,ncp);
  e->register_input(x);
...
int main(int, char* v[]) {
  int n=std::stoi(v[1]), ncp=std::stoi(v[2]);
  const double x0=1, T=1, eps=1e-15;            
  std::vector<DCO_A> p(n,1); 
  DCO_A x=x0;  
  DCO_TT* tape=DCO_TT::create();
  tape->register_variable(x); tape->register_variable(p);
  DCO_TPT tpos=tape->get_position();
  implicit_euler(x,p,T,eps,ncp);
