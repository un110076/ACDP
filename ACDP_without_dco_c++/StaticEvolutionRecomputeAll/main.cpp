...
struct StaticEvolutionRecomputeAllTarget : ACDP_EvolutionStep {
  ...
  StaticEvolutionRecomputeAllTarget(const double& dt, const int& nsteps) :
...
int main(int c, char* v[]) {
  ...
  std::unique_ptr<ACDP_StaticEvolutionRecomputeAll> e(
    new ACDP_StaticEvolutionRecomputeAll(new StaticEvolutionRecomputeAllTarget(dt,nsteps),1,1,nsteps)
  );
  e->register_input(x); 
  e->register_output(x); 
  e->link_target();
